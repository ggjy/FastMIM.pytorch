    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 shift_size=0 if ((i % 2 == 0) or (not shifted)) else window_size // 2,
                                 mlp_ratio=mlp_ratio,
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, block_size=32, shifted=True, center_rate=0., **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               use_checkpoint=use_checkpoint, shifted=shifted)
    ...
    def mim_block_masking(self, x, mask_ratio, block_size=32, center_rate=0.):
    ...
    def forward(self, x, mask_ratio, mask_token):
        mask, loss_mask = self.mim_block_masking(x, mask_ratio, block_size=self.block_size, center_rate=self.center_rate)
        B, C, H, W = x.shape
...
class PretrainSwin(nn.Module):    
    def __init__(self,
                 img_size=224, patch_size=16, decoder_embed_dim=512, decoder_depth=8, block_size=32,
                 mlp_ratio=4.,  qkv_bias=False,  qk_scale=None, drop_rate=0., embed_dim=96,
                 depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), window_size=7,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 use_learnable_pos_emb=False, norm_pix_loss=False, mim_loss='l2', **kwargs):
        super().__init__()
        self.img_size = img_size
        ...
        model_kwargs = dict(
            img_size=img_size, patch_size=4, block_size=block_size, window_size=window_size, embed_dim=embed_dim,
            depths=depths, num_heads=num_heads, **kwargs)

        self.encoder = SwinTransformer(pretrained=False, drop_path_rate=drop_path_rate, **model_kwargs)
        
        self.decoder_embed = nn.Linear(
            embed_dim * 2 ** (len(depths) - 1), decoder_embed_dim, bias=True) if decoder_depth > 0 else None
        
        self.decoder_blocks = nn.ModuleList([
            Block(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, norm_layer=nn.LayerNorm)
    ...
    def forward(self, imgs, mask_ratio=0.25, mask_type=None):
        # imgs = F.interpolate(imgs, size=self.img_size, mode='bicubic')
...
@register_model
def mim_swin_base(**kwargs):
    model = PretrainSwin(
        patch_size=32, embed_dim=128, num_heads=(4, 8, 16, 32),
        mlp_ratio=4, qkv_bias=True, norm_layer=None, drop_path_rate=0., **kwargs)
    return model

@register_model
def mim_swin_large(**kwargs):
    model = PretrainSwin(
        patch_size=32, embed_dim=192, num_heads=(6, 12, 24, 48),
        decoder_num_heads=16, mlp_ratio=4, qkv_bias=True, norm_layer=None, drop_path_rate=0., **kwargs)
    return model
